...
#define EXSNK_CIRCQ_SIZE  4UL

static BufMem dataPkts0[EXSNK_CIRCQ_SIZE];
CIRCQ_INDEX_DEFINE(bufQIdx0, EXSNK_CIRCQ_SIZE);
CIRCQ_DEFINE(bufQ_0, &bufQIdx0, EXSNK_CIRCQ_SIZE,
                  sizeof(BufMem), dataPkts0, CIRCQ_OVERWRITE_DISABLE);

static BufMem dataPkts1[EXSNK_CIRCQ_SIZE];
CIRCQ_INDEX_DEFINE(bufQIdx1, EXSNK_CIRCQ_SIZE);
CIRCQ_DEFINE(bufQ_1, &bufQIdx1, EXSNK_CIRCQ_SIZE,
                  sizeof(BufMem), dataPkts1, CIRCQ_OVERWRITE_DISABLE);

static BufMem dataPkts2[EXSNK_CIRCQ_SIZE];
CIRCQ_INDEX_DEFINE(bufQIdx2, EXSNK_CIRCQ_SIZE);
CIRCQ_DEFINE(bufQ_2, &bufQIdx2, EXSNK_CIRCQ_SIZE,
                  sizeof(BufMem), dataPkts2, CIRCQ_OVERWRITE_DISABLE);

static BufMem dataPkts3[EXSNK_CIRCQ_SIZE];
CIRCQ_INDEX_DEFINE(bufQIdx3, EXSNK_CIRCQ_SIZE);
CIRCQ_DEFINE(bufQ_3, &bufQIdx3, EXSNK_CIRCQ_SIZE,
                  sizeof(BufMem), dataPkts3, CIRCQ_OVERWRITE_DISABLE);

#define MAX_AVB_DUMP_SIZE (80*1024)

uint8_t COMP_SECTION(".dma.bcm.test") Avb1722DumpArray0 [MAX_AVB_DUMP_SIZE];
uint32_t ExSnk_Port0DataSize = 0UL;
static uint32_t ExSnk_DoneCnt0 = 0UL;

uint8_t COMP_SECTION(".dma.bcm.test") Avb1722DumpArray1 [MAX_AVB_DUMP_SIZE];
uint32_t ExSnk_Port1DataSize = 0UL;
static uint32_t ExSnk_DoneCnt1 = 0UL;

uint8_t COMP_SECTION(".dma.bcm.test") Avb1722DumpArray2 [MAX_AVB_DUMP_SIZE];
uint32_t ExSnk_Port2DataSize = 0UL;
static uint32_t ExSnk_DoneCnt2 = 0UL;

uint8_t COMP_SECTION(".dma.bcm.test") Avb1722DumpArray3 [MAX_AVB_DUMP_SIZE];
...
static void ExSnk_FillBufferDone0(MSYS_BufferType *const aMediaBuffer)
{
    ...
    if ((aMediaBuffer->buffer[0].size != 0UL) &&
        ((ExSnk_Port0DataSize+aMediaBuffer->buffer[0].size) < MAX_AVB_DUMP_SIZE)){

        (void)memcpy(&Avb1722DumpArray0[ExSnk_Port0DataSize],
...
static void ExSnk_FillBufferDone1(MSYS_BufferType *const aMediaBuffer)
{
    ...
    if ((aMediaBuffer->buffer[0].size != 0UL) &&
        ((ExSnk_Port1DataSize+aMediaBuffer->buffer[0].size) < MAX_AVB_DUMP_SIZE)){

        (void)memcpy(&Avb1722DumpArray1[ExSnk_Port1DataSize],
                     aMediaBuffer->buffer[0].data,
                     aMediaBuffer->buffer[0].size);
...
static void ExSnk_FillBufferDone2(MSYS_BufferType *const aMediaBuffer)
{
    ...
    if ((aMediaBuffer->buffer[0].size != 0UL) &&
        ((ExSnk_Port2DataSize+aMediaBuffer->buffer[0].size) < MAX_AVB_DUMP_SIZE)){

        (void)memcpy(&Avb1722DumpArray2[ExSnk_Port2DataSize],
                     aMediaBuffer->buffer[0].data,
                     aMediaBuffer->buffer[0].size);
...
static void ExSnk_FillBufferDone3(MSYS_BufferType *const aMediaBuffer)
{
    ...
    if ((aMediaBuffer->buffer[0].size != 0UL) &&
        ((ExSnk_Port3DataSize+aMediaBuffer->buffer[0].size) < MAX_AVB_DUMP_SIZE)){

        (void)memcpy(&Avb1722DumpArray3[ExSnk_Port3DataSize],
                     aMediaBuffer->buffer[0].data,
                     aMediaBuffer->buffer[0].size);
...
TASK(ExSnk_DataProcessTask)
{
    ...
    for (i = 0UL; i < NUM_INPUT_BUFFERS; i++) {
        buf.bufIdx = i;
        ULOG_ERR("Init : %d\n",buf.bufIdx);
