...
/* BRCM AVB Test Data Config */
#define BRCM_AVB_TESTDATA_CONFIG (0xFF00UL)

MSYS_BufferType ExSrc_MediaBuffer[EXSRC_MAX_NUM_OUT_PORT][MSGQ_SIZE];

#define AVBT_TEST_MAX_NAL_DATA_SIZE (500*1024)
static uint8_t COMP_SECTION(".dma.bcm.test") enc_data_array [AVBT_TEST_MAX_NAL_DATA_SIZE];
...
/* API to get a NAL unit from the H264 data buffer */
...
void ExSrc_ProcessCmd(void)
{
    ...
    if (BCM_ERR_OK == retVal) {
        ...
        case MSYS_COMP_CMD_CONFIG: {
            ...
            retVal = IMGL_LoadImg(BRCM_AVB_TESTDATA_CONFIG,
                                 enc_data_array,
                                 0UL, AVBT_TEST_MAX_NAL_DATA_SIZE, SystemEvent2, &imglStatus);
            if (BCM_ERR_OK != retVal) {
                ULOG_ERR("ExSrc_ProcessCmd > IMGL_LoadImg(%d) failed \n", retVal);
                break;
            }
            retVal = BCM_WaitEvent(SystemEvent2);
            if (BCM_ERR_OK != retVal) {
                ULOG_ERR("ExSrc_ProcessCmd >  BCM_WaitEvent Failed %d\n",retVal);
                break;
            }
            retVal = BCM_ClearEvent(SystemEvent2);
            if (BCM_ERR_OK != retVal) {
...
volatile uint32_t AlarmVal[4] = {0UL};
static uint32_t counter = 0UL;

uint32_t ExSrc_Events[4UL] = {
SystemEvent8,
SystemEvent9,
SystemEvent10,
SystemEvent11,
...
{
    uint32_t i = (counter++) % 4;

    ATOMIC_Set(&AlarmVal[i], 1UL);
    SetEvent(ExSrc_DataProcessTask, ExSrc_Events[i]);
}
...
void ExSrc_ProcessData_Port0(const MSYS_OutPortType * const outPort)
{
    ...
    localAlarmVal = ATOMIC_Set(&AlarmVal[0], 0UL);
...
void ExSrc_ProcessData_Port1(const MSYS_OutPortType * const outPort)
{
    ...
    retVal = MSGQ_GetMsgIdx(&outPort->dataQ, &mediaBufferIdx);
    localAlarmVal = ATOMIC_Set(&AlarmVal[1], 0UL);
...
void ExSrc_ProcessData_Port2(const MSYS_OutPortType * const outPort)
{
    ...
    retVal = MSGQ_GetMsgIdx(&outPort->dataQ, &mediaBufferIdx);
    localAlarmVal = ATOMIC_Set(&AlarmVal[2], 0UL);
...
void ExSrc_ProcessData_Port3(const MSYS_OutPortType * const outPort)
{
    ...
    retVal = MSGQ_GetMsgIdx(&outPort->dataQ, &mediaBufferIdx);
    localAlarmVal = ATOMIC_Set(&AlarmVal[3], 0UL);
