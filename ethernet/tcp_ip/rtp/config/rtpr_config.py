    ...
    def write(self, outfile, cfg):
        ...
        if hasattr(cfg, 'rtp_port'):
            for each in cfg.rtp_port:
                fil.write(struct.pack('I', getMediaFormat(each.mediaFormat)))
                fil.write(struct.pack('I', getHexValue(each.SSRC)))
                fil.write(struct.pack('I', getpayloadFormat(each.payloadFormat)))
                fil.write(struct.pack('I', getUDPPayloadID(each.rtpPayloadID)))
                fil.write(struct.pack('H', rsv))
                fil.write(struct.pack('H', rsv))
                fil.write(struct.pack('I', rsv))
                fil.write(struct.pack('I', getUDPPayloadID(each.rtcpPayloadID)))
        fil.close()
...
class XMLParser:
    ...
    def processCompXML(self, compfile, outfile):
        ...
        rtpr = rtpr_config_t()
        rtpr.write(outfile, cfg)
        print outfile+' is generated'

# ******************************************************************************
def usage():
    print("python rtpr_config.py -i <input> -o <output> -s <schema>")
    sys.exit(1)
