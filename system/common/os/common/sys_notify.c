...
/**
    ...
    @file system/common/os/common/system.c
...
*/
#define BRCM_SWDSGN_SYS_CONTEXT_GLOBAL       (0x80U)   /**< @brief #COMP_Context        */
#define BRCM_SWDSGN_SYS_FUNCTION_PART_PROC   (0x81U)   /**< @brief #COMP_FunctionPart   */
#define BRCM_SWDSGN_SYS_FUNCTION_PROC        (0x82U)   /**< @brief #COMP_Function       */
#define BRCM_SWDSGN_SYS_INT_STATE_MACRO      (0x83U)   /**< @brief #COMP_IntStateType   */
#define BRCM_SWDSGN_SYS_CONTEXT_DEFINE_MACRO (0x84U)   /**< @brief #COMP_CONTEXT_DEFINE */
#define BRCM_SWDSGN_SYS_INTCONTEXT_TYPE      (0x85U)   /**< @brief #COMP_IntContextType */
#define BRCM_SWDSGN_SYS_INTCONTEXT_GLOBAL    (0x86U)   /**< @brief #COMP_IntContext     */
/** @} */
...
*/
typedef struct _SYS_SubsystemNotifType {
    SYS_SubsystemIDType id;
    SYS_SubsystemStateType state;
} SYS_SubsystemNotifType;
...
/**
    ...
    @trace #BRCM_SWREQ_COMP_FUNCTIONALITY
*/
SYS_SubsystemNotifType SYS_SubsystemNotif[MSGQ_SIZE];
...
void* SYS_GetNotif(uint32_t idx)
{
    ...
    if (idx < MSGQ_SIZE) {
        pRet = (void *)(&SYS_SubsystemNotif[idx]);
    }
    return pRet;
}
...
MSGQ_DEFINE((SYS_NotifQ), (SYS_TASK_NAME), (SYS_NOTIF_EVENT),
            (MSGQ_CLIENT_SIGNAL_DISABLE), SYS_SubsystemNotifType, (&SYS_NotifHdrQ),
            (SYS_GetNotif));


SYS_SubsystemStateType SYS_SubsystemState[SYS_SUBSYSTEM_MAX] = {SYS_SUBSYSTEM_STATE_UNINIT};

/** @} */

/**
    @addtogroup grp_system_sw
    @{
*/
...
/**
    ...
    @endcode
*/
void SYS_NotifyState(SYS_SubsystemIDType aID, SYS_SubsystemStateType aState)
{
    SYS_SubsystemNotifType notif;
    if ((aID < SYS_SUBSYSTEM_MAX) && (aState <= SYS_SUBSYSTEM_STATE_ERROR)) {
        notif.id = aID;
        notif.state = aState;
        (void)MSGQ_SendMsg(&SYS_NotifQ, &notif, MSGQ_NO_CLIENTMASK, NULL);
    }
}
...
/**
    ...
    @endcode
*/
SYS_SubsystemStateType SYS_GetState(SYS_SubsystemIDType aID)
{
    uint32_t idx;
    ...
    do {
        ...
        if (BCM_ERR_OK == retVal) {
            id = SYS_SubsystemNotif[idx].id;
            if (id < SYS_SUBSYSTEM_MAX) {
                SYS_SubsystemState[id] = SYS_SubsystemNotif[idx].state;
            }
            retVal = MSGQ_CompleteMsgIdx(&SYS_NotifQ, idx);
        }
    } while (BCM_ERR_OK == retVal);

    state = SYS_SUBSYSTEM_STATE_UNINIT;
    if (aID < SYS_SUBSYSTEM_MAX) {
        state = SYS_SubsystemState[aID];
    }
    return state;
}
