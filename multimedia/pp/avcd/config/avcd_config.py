...
class avcd_config_t:
    def __init__(self, w, h, prof, lvl,num_pp):
        self.magic_number = 0x45435641
        self.version = sys.getsizeof(avcd_config_t) - 4
        self.width = (w + 15) & (~15)
        self.height = (h + 15) & (~15)
        self.profile = prof
        self.level = lvl
    ...
    def write(self, outfile):
        ...
        fil.write(struct.pack('I',self.num_pp))
        fil.close()
...
class XMLParser:
    ...
    def processCompXML(self, compfile, outfile):
        ...
        if cfg is None:
            ...
            return
        avcd = avcd_config_t(cfg.width,cfg.height,cfg.profile,cfg.level,cfg.num_ppcores)
        avcd.write(outfile)
        print outfile+' is generated'

# ******************************************************************************
def usage():
    print("python avcd_config.py -i <input> -o <output> -s <schema>")
    sys.exit(1)
