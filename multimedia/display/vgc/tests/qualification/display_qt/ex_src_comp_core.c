...
static MSYS_BufferType ExSrc_MediaBufferVideo[MSGQ_SIZE];
static void* ExSrc_GetDataMsgVideo(uint32_t i)
{
    void *dataMsg;
    if (MSGQ_SIZE > i) {
...
static MSYS_BufferType ExSrc_MediaBufferGfx[MSGQ_SIZE];
static void* ExSrc_GetDataMsgGfx(uint32_t i)
{
    void *dataMsg;
    if (MSGQ_SIZE > i) {
...
static MSYS_CmdType ExSrc_CmdMsgQ[MSGQ_SIZE];
...
static uint32_t ExSrc_GetNumMediaOutPort(void)
{
...
static void ExSrc_ProcessCmd(void)
{
    int32_t retVal;
...
extern void ExSrc_DataProcessTask_Cb(void)
{
    StatusType status;
    (void)ATOMIC_Set(&AlarmVal, 1UL);
...
static void fill_rect(uint32_t aX1, uint32_t aY1, uint32_t aX2, uint32_t aY2, uint8_t *aBuff)
{
    /* YUV *yuv = (YUV *)aBuff; */
    uint32_t i,j;
...
static void ExSrc_ProcessData(const MSYS_OutPortType * const outPort)
{
    int32_t retVal;
    ...
    if (BCM_ERR_OK != retVal) {
        ...
        if (outPort->portIndex == 0UL) {
            ...
            if ((NULL != mediaBuffer->buffer[0].data)
                && (0UL != mediaBuffer->buffer[0].size)){
                if (0UL != (mbRowNumGfx & 4UL)) {
                    (void)memset(mediaBuffer->buffer[0].data, 0x00, mediaBuffer->buffer[0].size);
                } else {
                    (void)memset(mediaBuffer->buffer[0].data, 0xFF, mediaBuffer->buffer[0].size);
                }
                DCACHE_CleanAll();
