...
/******************************************************************************
 File Name: vidxmtr.c
 Descritpion: This file implememnts video transmitter application
******************************************************************************/
...
static const MSYS_Type mediaSys = {
    ...
    .component[1].config[0] = MSYS_CONFIG_VENC_MOD + 1U,
    .component[1].config[1] = MSYS_CONFIG_VENC_FW0 + 1U,
    .component[1].config[2] = MSYS_CONFIG_VENC_FW1 + 1U,
#else
...
void Shell_App(uint32_t aConsoleID, char* aArgString, uint32_t aArgStringLen)
{
    ...
    if (NULL != aArgString) {
        if (0 == strncmp(aArgString, "start", CMD_STR_LEN)) {
            ...
            ret = BCM_SetEvent(VidXmtr, SystemEvent0);
            if (BCM_ERR_OK != ret) {
