...
/**
    ...
    @{
    @file viewfinder.c
    @brief This file implements the view finder application
...
static const MSYS_Type mediaSys = {
    ...
    .component[1].compID = MSYS_COMP_ID_GFXSRC,
    .component[1].numConfigs = 1UL,
    .component[1].config[0] = 0U,
    .component[1].portMap[0].srcCompIdx = 0U,
    .component[1].portMap[0].srcPortIdx = 0U,

    .component[2].compID = MSYS_COMP_ID_VGCC,
...
{
    int32_t ret;
    ret = BCM_ActivateTask(ViewFinder);
    if (BCM_ERR_OK != ret) {
...
void Shell_App(uint32_t aConsoleID, char* aArgString, uint32_t aArgStringLen)
{
    ...
    if (NULL != aArgString) {
        if (0 == strncmp(aArgString, "start", CMD_STR_LEN)) {
            ...
            ret = BCM_SetEvent(ViewFinder, SystemEvent0);
            if (BCM_ERR_OK != ret) {
