...
static const MSYS_Type mediaSys = {
    ...
    .component[1].compID = MSYS_COMP_ID_APLY,
    .component[1].numConfigs = 1UL,
    .component[1].config[0] = 0UL,
    .component[1].portMap[0].srcCompIdx = 0U,
    .component[1].portMap[0].srcPortIdx = 1U,

    .component[2].compID = MSYS_COMP_ID_AVCD,
    .component[2].numConfigs = 2UL,
    .component[2].config[0] = MSYS_CONFIG_AVCD_MOD,
    .component[2].config[1] = MSYS_CONFIG_AVCD_FW0,
...
TASK(AVRcvr)
{
    ...
    BCM_GetTime(&t1);
    ULOG_ERR("Audio Video Transceiver started at %u sec %u msec from start\n",
...
{
    int32_t ret;
    ret = BCM_ActivateTask(AVRcvr);
    if (BCM_ERR_OK != ret) {
        ULOG_INFO("Failed to send start signal\n");
    }
}
...
void Shell_App(uint32_t aConsoleID, char* aArgString, uint32_t aArgStringLen)
{
    ...
    if (NULL != aArgString) {
        if (0 == strncmp(aArgString, "start", CMD_STR_LEN)) {
            ...
            ret = BCM_SetEvent(AVRcvr, SystemEvent0);
            if (BCM_ERR_OK != ret) {
                CONSOLE_Write(aConsoleID, "Failed to send stop signal\n");
            }
        } else {
            CONSOLE_Write(aConsoleID,
                    "Invalid argument. Usage: app start/stop\n");
        }
    } else {
        CONSOLE_Write(aConsoleID,
                "No argument mentioned. Usage: app start/stop\n");
    }
}
