...
/**
    ...
    @{
    @file rtpvidrcvr.c
    @brief This file implements the wifi RTP video receiver application
...
TASK(RtpVidRcvr)
{
    ...
    cache->valid = 1UL;
    cache->ipAddr = (INET_CONVERT_IP_ADDRESS((192U), (168U), (10U), (2U)));
...
{
    int32_t ret;
    ret = BCM_ActivateTask(RtpVidRcvr);
    if (BCM_ERR_OK != ret) {
...
void Shell_App(uint32_t aConsoleID, char* aArgString, uint32_t aArgStringLen)
{
    ...
    if (NULL != aArgString) {
        if (0 == strncmp(aArgString, "start", CMD_STR_LEN)) {
            ...
            ret = BCM_SetEvent(RtpVidRcvr, SystemEvent0);
            if (BCM_ERR_OK != ret) {
