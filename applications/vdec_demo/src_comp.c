 ...
 *****************************************************************************/
#define SRC_MAX_BS_SIZE (0x500000UL)
uint8_t COMP_SECTION(".noload.data") SRC_BsBuffer[SRC_MAX_BS_SIZE];

#define SRC_MAX_BS_NALS (90UL)
uint32_t SRC_NalOffsets[SRC_MAX_BS_NALS];
...
static void ExSrc_ResetStateHandler(void)
{
    ...
    if (BCM_ERR_OK == retVal) {
        ...
        if (MSYS_COMP_CMD_INIT == processCmd->cmd) {
            ...
            ExSrc_Comp.bsBufSize = 0UL;
            ExSrc_Comp.numNals = 0UL;
            ExSrc_Comp.curNalIdx = 0UL;
            ExSrc_Comp.currOffset = 0UL;
            ExSrc_Comp.dataBufCnt = 0UL;
...
static uint32_t SRC_LoadTestImg(uint16_t aImgID, uint8_t* aBuffer, uint32_t aBufSize)
{
    int32_t ret;
...
int32_t SRC_FindResync(unsigned char* bs,uint32_t size)
{
    int32_t i;
    int32_t offset = -1;
...
uint32_t SRC_ComputeNals(uint8_t *aBuffer, uint32_t aBufSize)
{
    ...
    remainingSize = aBufSize;
    currOffset = 0;
    while ((0 < remainingSize) &&
           (SRC_MAX_BS_NALS > numNals)) {
        /* send the SPS and PPS */
        currSize = SRC_FindResync(&aBuffer[currOffset],
                              remainingSize);
        if (currSize == -1) {
            SRC_NalOffsets[numNals++] = remainingSize;
            break;
        } else {
            SRC_NalOffsets[numNals++] = currSize;
            currOffset += currSize;
...
static void ExSrc_InitStateHandler(void)
{
    ...
    if (BCM_ERR_OK == retVal) {
        ...
        case MSYS_COMP_CMD_CONFIG:
            ...
            ExSrc_Comp.bsBufSize = 0UL;
            /* load the stream */
            ExSrc_Comp.bsBufSize += SRC_LoadTestImg(processCmd->configID, &SRC_BsBuffer[ExSrc_Comp.bsBufSize], SRC_MAX_BS_SIZE - ExSrc_Comp.bsBufSize);
...
static void ExSrc_ReadyStateHandler(void)
{
    ...
    if (BCM_ERR_OK == retVal) {
        ...
        case MSYS_COMP_CMD_START:
            ...
            ExSrc_Comp.currOffset = 0UL;
            ExSrc_Comp.dataBufCnt = 0UL;
            ExSrc_Comp.frameCount = 0UL;
            /* Compute the nals */
            ExSrc_Comp.numNals = SRC_ComputeNals(SRC_BsBuffer, ExSrc_Comp.bsBufSize);
            BCM_GetTime(&ExSrc_Comp.timeStamp);
...
static void ExSrc_RunStateHandler(void)
{
     ...
     * source component.
     */
    while (EXSRC_STATE_RUNNING == ExSrc_Comp.state) {
        ...
        if (retVal == BCM_ERR_OK) {
            ...
            if ((NULL != mediaBufferPtr->buffer[0].data) && (0UL != mediaBufferPtr->buffer[0].size)) {
                /* Fill the buffer */
                mediaBufferPtr->buffer[0].size = SRC_NalOffsets[ExSrc_Comp.curNalIdx++];
                BCM_MemCpy(mediaBufferPtr->buffer[0].data,&SRC_BsBuffer[ExSrc_Comp.currOffset], mediaBufferPtr->buffer[0].size);
                ExSrc_Comp.currOffset += mediaBufferPtr->buffer[0].size;
                if (ExSrc_Comp.numNals <= ExSrc_Comp.curNalIdx) {
                    ExSrc_Comp.curNalIdx = 0UL;
                    ExSrc_Comp.currOffset = 0UL;
                }
            ...
            if (32UL < mediaBufferPtr->buffer[0].size) {
                BCM_TimeType localTime;
                BCM_GetTime(&localTime);
                ExSrc_Comp.frameCount++;
                if(ExSrc_Comp.timeStamp.s != localTime.s) {
                    ULOG_INFO("FPS: %d  \n", ExSrc_Comp.frameCount);
                    ExSrc_Comp.frameCount = 0UL;
                    ExSrc_Comp.timeStamp = localTime;
                }
            }

            ExSrc_Comp.dataBufCnt++;

            retVal = MSGQ_CompleteMsgIdx(&ExSrc_OutPort.dataQ, mediaBufIdx);
            if (BCM_ERR_OK != retVal) {
                ULOG_ERR("MSGQ_CompleteMsgIdx Error: %d  \t line %d \n",retVal,__LINE__);
                break;
            }
        } else {
            break;
