...
/**
    ...
    @file avxmtr.c
    @brief This file implements the audio video transmitter application with gPTP
            enabled
...
static const MSYS_Type mediaSys = {
    ...
    .component[3].compID = MSYS_COMP_ID_AREC,
...
void Shell_App(uint32_t aConsoleID, char* aArgString, uint32_t aArgStringLen)
{
    ...
    if (NULL != aArgString) {
        if (0 == strncmp(aArgString, "start", CMD_STR_LEN)) {
            ...
            ret = BCM_SetEvent(AVXmtr, SystemEvent0);
            if (BCM_ERR_OK != ret) {
